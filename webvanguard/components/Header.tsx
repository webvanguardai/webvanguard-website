    ...
    >
      <div className="max-w-[90rem] mx-auto px-6 md:px-12 flex items-center justify-between h-16 md:h-20">
        <Link href="/" className="relative z-50">
          <span className="font-display font-black text-xl tracking-tight text-text-primary uppercase">
        ...
        <nav className="hidden md:flex items-center gap-10">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-[13px] text-text-secondary hover:text-accent transition-colors duration-300 uppercase tracking-wider font-body"
            >
              {link.label}
            </a>
          ))}
            ...
            <nav className="flex flex-col items-center gap-8">
              {navLinks.map((link, i) => (
                <motion.a
                  ...
                  className="text-4xl font-display font-black text-text-primary hover:text-accent transition-colors uppercase"
                >
                  {link.label}
                </motion.a>
              ))}
